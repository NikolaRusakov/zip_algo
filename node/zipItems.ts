...
const seqTwoFn = ({seqOne = [], next = true, acc = []}: SeqTwoAccumulator) => ({
    seqOne: seqOne,
    acc: acc,
    next,
    valueOf: () => ({
    ...
    fmap: (f: (stf: SeqTwoAccumulator) => SeqTwoAccumulator) => seqTwoFn(f({
        seqOne,
        acc,
        next
    }))
...
export const insertItemAtOffset = ({acc, seqOne}: SeqTwoAccumulator) =>
    ({offset, period}: { period: number, offset: number }): SeqTwoAccumulator => {
        if (offset < seqOne.length) {
            const newSeqOne = [...seqOne.slice(0, offset), offset, ...seqOne.slice(offset)]
 ...
 */
export const processNextInsertion = ({acc, next, seqOne}: SeqTwoAccumulator) => (period: number): SeqTwoAccumulator => {
    const nextInsertionIndex = seqOne[tail(acc)] + period
    const notTail = nextInsertionIndex < seqOne.length

    if (notTail) {
        const nextSeqOne = [...seqOne.slice(0, tail(acc) + period),
            nextInsertionIndex,
 ...
 * */
const preprocessInsertionList = (sequenceOne: number[]) => (period: number) => (offset: number = 2): SeqTwoAccumulator => {
    /**
 ...
 * */
const overrideWithGeneratedItem = (acc: number[], next: number, index: number) => (sequenceTwo: number[]): number[] => {
