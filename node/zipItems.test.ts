...
import {insertItemAtOffset, processNextInsertion, zipItems} from "./zipItems.ts";
...
Deno.test("case #1", function (): void {
    ...
    const zips = zipItems(
        primarySeqGenerator(10), primarySeqGenerator, 7, 2)
    assertEquals(zips, expects);
});

Deno.test("case #2", function (): void {
    const expects = [1000, 1001, 0, 1002, 1003, 1, 1004, 1005, 2, 1006, 1007, 3, 1008, 1009, 4, 1010, 1011, 5, 1012, 1013, 6, 1014, 1015, 7, 1016, 1017, 8, 1018, 1019, 9, 1020, 1021, 10, 1022]
    const zips = zipItems(
        primarySeqGenerator(23), primarySeqGenerator, 2, 3)
    assertEquals(zips, expects);
});
...
Deno.test("case #3", function (): void {
    ...
    const zips = zipItems(
        primarySeqGenerator(5), primarySeqGenerator, 4, 6)
    assertEquals(zips, expects);
});
...
Deno.test("case #4", function (): void {
    ...
    const zips = zipItems(
        primarySeqGenerator(5), primarySeqGenerator, 0, 3)
    assertEquals(zips, expects);
});
...
Deno.test("case #5", function (): void {
    ...
    const zips = zipItems(
        primarySeqGenerator(5), primarySeqGenerator, 1, 3)
    assertEquals(zips, expects);
});

Deno.test("case #6", function (): void {
    const expects = [1000, 1001, 1002, 0, 1003, 1004, 1005, 1006, 1007, 1008, 1, 1009, 1010, 1011, 1012, 1013, 1014, 2, 1015, 1016, 1017, 1018, 1019, 1020, 3, 1021, 1022, 1023, 1024, 1025, 1026, 4, 1027, 1028, 1029, 1030, 1031, 1032, 5, 1033, 1034, 1035, 1036, 1037, 1038, 6, 1039, 1040, 1041, 1042, 1043, 1044, 7, 1045, 1046, 1047, 1048, 1049]
    const zips = zipItems(
        primarySeqGenerator(50), primarySeqGenerator, 3, 7)
    assertEquals(zips, expects);
});
...
Deno.test("case offset start at end", function (): void {
    ...
    const zips = zipItems(
        primarySeqGenerator(5), primarySeqGenerator, 5, 2)
    assertEquals(zips, expects);
});
...
Deno.test("case offset out of bounds", function (): void {
    const expects = [1000, 1001, 1002, 1003, 1004]
    const zips = zipItems(
        primarySeqGenerator(5), primarySeqGenerator, 6, 2)
    assertEquals(zips, expects);
});
...
Deno.test("insertItemAtOffset start at the end", function (): void {
    const expects = [1000, 1001, 1002, 1003, 1004]
    const inserts = insertItemAtOffset({seqOne: expects, acc: [], next: true})({offset: 5, period: 2})
...
Deno.test("insertItemAtOffset start at OoB", function (): void {
    const expects = [1000, 1001, 1002, 1003, 1004]
    const inserts = insertItemAtOffset({seqOne: expects, acc: [], next: true})({offset: 10, period: 2})
    assertEquals(inserts.seqOne, expects);
    assertEquals(inserts.next, false);
});
